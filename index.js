...
class Email {
    ...
    createRecentlyOrder({items, orderId, orderStatus, products}) {
        ...
        const testTemplateMjml = fs.readFileSync(path.join(__dirname, 'product', 'product.hbs'), 'utf8')
        const linkButtonMjml = fs.readFileSync(path.join(__dirname, 'common', 'linkButton.hbs'), 'utf8')

        const orderBodyTemplate = Handlebars.compile(orderBodyMjml)
        const productsTemplate = Handlebars.compile(productsMjml)
        const testTemplate = Handlebars.compile(testTemplateMjml)
        const linkButtonTemplate = Handlebars.compile(linkButtonMjml)

        Handlebars.registerPartial('Body', orderBodyTemplate({orderId, orderStatus}) + productsTemplate({items, ...successOrder}) + testTemplate({products}));
        Handlebars.registerPartial('LinkButton', linkButtonTemplate({buttonText}) );
...
const t = email.createRecentlyOrder({items: orders, orderStatus: 'CLOSED', orderId: '№295034', products: orders})
